...
const Auth: React.FC<AuthProps> = ({ onLogin, onClose }) => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setLoading(true);
    const cleanEmail = email.trim(); // Trim spaces
    setTimeout(() => {
      onLogin({
        id: `u-${Date.now()}`,
        name: cleanEmail.split('@')[0],
        email: cleanEmail,
        photo: `https://ui-avatars.com/api/?name=${cleanEmail}&background=2563EB&color=fff`,
        streak: 0,
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-200 dark:bg-slate-900/50 backdrop-blur-sm p-4">
      <div className="w-full max-w-lg h-full max-h-[900px] bg-white dark:bg-slate-950 overflow-hidden relative shadow-2xl rounded-[3rem] border-4 border-slate-300 dark:border-slate-800 flex flex-col">
