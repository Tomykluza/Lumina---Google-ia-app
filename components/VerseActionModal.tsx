...
const VerseActionModal: React.FC<VerseActionModalProps> = ({ 
  verse, onClose, onHighlight, onSaveNote, currentHighlight, currentNote 
}) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center">
      ...
      <div className="relative bg-white w-full rounded-t-3xl max-w-lg shadow-2xl overflow-hidden animate-in slide-in-from-bottom duration-300">
        <div className="h-1 w-12 bg-slate-200 rounded-full mx-auto mt-3 mb-4" />
        
        <div className="px-6 pb-8 space-y-6">
          <div className="text-center">
            <h4 className="text-xs font-bold text-slate-400 uppercase mb-1">{verse.book} {verse.chapter}:{verse.number}</h4>
            <p className="text-slate-800 font-serif italic text-lg leading-snug">"{verse.text}"</p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 min-h-[100px]"
            />
            <button
            ...
            {generatedImage ? (
              <div className="relative group rounded-xl overflow-hidden shadow-lg">
                ...
                <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/80 to-transparent flex flex-col items-center">
                  <p className="text-white text-xs font-serif italic text-center mb-3">"{verse.text}"</p>
                  <div className="flex gap-2">
                    <button 
                      onClick={downloadImage}
                      className="bg-white/20 backdrop-blur-md text-white text-xs font-bold px-4 py-2 rounded-full border border-white/30 hover:bg-white hover:text-black transition-colors"
                    >
                      Descargar
                    </button>
                    <button 
                      onClick={() => setGeneratedImage(null)}
                      className="bg-white/20 backdrop-blur-md text-white text-xs font-bold px-4 py-2 rounded-full border border-white/30 hover:bg-red-500 transition-colors"
                    >
                      Nueva
                    </button>
                  </div>
              ...
              <button
                ...
                className="w-full bg-blue-600 text-white font-bold py-4 rounded-xl flex items-center justify-center gap-2 hover:bg-blue-700 transition-all active:scale-95 disabled:opacity-50"
              >
                {isGenerating ? (
